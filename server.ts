...
async function setupWebhook(req: express.Request) {
  ...
  try {
    ...
    console.log('Error registering Telegram webhook automatically:', err.message);
  }
}
...
async function handleTelegramMessage(body: any) {
  ...
  if (cleanText.includes('variasi 1') || cleanText === 'variasi satu' || cleanText === '/v1' || cleanText === 'v1' || cleanText.includes('🎬 variasi 1')) {
    runVariationSequence(1).catch((err) => console.log('Err running v1 from TG:', err.message));
    return;
  }

  // 4. RUN VARIATION 2
  if (cleanText.includes('variasi 2') || cleanText === 'variasi dua' || cleanText === '/v2' || cleanText === 'v2' || cleanText.includes('🎬 variasi 2')) {
    runVariationSequence(2).catch((err) => console.log('Err running v2 from TG:', err.message));
    return;
  }
...
apiRouter.use((req, res, next) => {
  ...
  setupWebhook(req).catch((err) => console.log('Telegram Webhook Setup Error in dev:', err.message));
  next();
});
...
apiRouter.post('/config', async (req, res) => {
  ...
  setupWebhook(req).catch((err) => console.log('Immediate configuration webhook setup error:', err.message));
...
apiRouter.post('/telegram/webhook', async (req, res) => {
  try {
    ...
    console.error('Webhook processing error in dev:', err.message);
  }
  res.json({ ok: true });
});
